...
**Download and install the [ShotAI desktop client](https://www.shotai.io) first.** Create a collection for the business, import videos the owner has the right to use, wait for shot analysis and indexing, then enable MCP and connect the agent. Keep the client and MCP service available during use. Confirm the collection with `list_collections`, then run one `search_shots` query restricted to it. See [Setup](setup.md). Files stored only on disk, installing this skill, or supplying paths do not make footage searchable. For connection failures or empty results, check client/service availability, collection selection, import completion and indexing first.

Prefer ShotAI MCP tools already available in the host. Use `shotai_client.py` only as a local SSE fallback. It requires Python 3.10+ with no third-party packages. Set `SHOTAI_URL` for a different loopback port and use `SHOTAI_TOKEN` or an explicitly supplied `--token-file` for authentication; the helper never searches local databases or stored application credentials. Credentials are redacted from JSON output, including JSON encoded in MCP text blocks.

Run `tools/list --output FILE` for the current tool list, then `call TOOL --args-json FILE --output FILE`. The result envelope stores fresh call timestamps, methods, arguments, status, and initialization evidence. Follow `nextCursor` for paginated tool lists. Returned `isError`, JSON-RPC errors, structured errors, and partial shot-export failures produce a nonzero exit code. Timeouts do not cancel actions already accepted by the server.

Use real collection and shot IDs from the user's library. Search within the current business's collection, inspect candidate frames, and export selected shots into the current project's output directory. The parameter table above is a dated compatibility reference from a previously inspected ShotAI build, not an advertised schema and not a guarantee about current versions. Save the fresh tool list and disclose that fallback basis whenever the current schema is incomplete. Never present old cached results as a live MCP call.
