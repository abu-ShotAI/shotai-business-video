...
[English](../setup.md) · [中文完整流程](../workflow.zh.md)

## 先准备 ShotAI 素材库

**下载客户端：[ShotAI 官网](https://www.shotai.io)**
...
把整个 shotai-business-video 目录放进技能目录，例如 ~/.codex/skills/shotai-business-video/。默认使用英文包，中文包是可选说明入口；两包同名同运行脚本，只选择其一安装，不重复放入发现路径。每包都有另一语言完整说明。日常沟通跟随用户语言，视频输出独立选中文（`zh`）、英文（`en`）或分别两版（`both`），平台也独立选择，文档语言不限制视频语言。新会话调用 $shotai-business-video。
...
Windows 使用 `.venv\Scripts\python.exe`。缺少 FFmpeg/FFprobe 或系统字体时单独安装；付费 TTS 提供商不会自动配置。
